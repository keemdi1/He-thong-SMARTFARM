...
#include <DHT.h>
#define digitalWriteFast(pin, value) gpio_set_level((gpio_num_t)(pin), (value))
...
// ========== NGƯỠNG & LỌC NHIỄU ==========
#define DOAM_DAT_KHO 3000
#define DOAM_DAT_UOT 1000
...
void setup() {
  ...
  Serial.println("\nĐã kết nối WiFi!");
  Serial.print("IP: ");
  Serial.println(WiFi.localIP());// địa chỉ ip wifi đã kết nối
...
float nhietdo = 0;
float doamkk = 0;
int doamdat_percent = 0;
int anhsang_percent = 0;

unsigned long lastDocCamBienTime = 0;
const unsigned long docCamBienInterval = 1000;  // đọc cảm biến mỗi 1000ms (1 giây)

// ========== VÒNG LẶP ==========
void loop() {
  if (millis() - lastDocCamBienTime >= docCamBienInterval) {
    lastDocCamBienTime = millis();  // cập nhật thời điểm đọc mới
    docThongSoMoiTruong();          // gọi hàm đọc cảm biến
  }
  if (isAuto) {
    dieuKhienTuDong();
  } else {
...
void docThongSoMoiTruong() {
  ...
  doamdat_percent = map(doamdat_raw, DOAM_DAT_KHO, DOAM_DAT_UOT, 0, 100);
  doamdat_percent = constrain(doamdat_percent, 0, 100);

  anhsang_percent = map(anhsang_raw, ANHSANG_TOI, ANHSANG_SANG, 0, 100);
...
void dieuKhienTuDong() {
  ...
  digitalWrite(DEN_PIN, anhsang_percent < 30 ? HIGH : LOW);
...
void dieuKhienThuCong() {
  Serial.println(">>> Chế độ: THỦ CÔNG");

  digitalWriteFast(FAN_PIN, getFirebaseInt("/thietbi5/quat"));
  digitalWriteFast(DEN_PIN, getFirebaseInt("/thietbi4/den"));
